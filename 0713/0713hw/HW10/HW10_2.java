...
class Student{
	void report(Subject s){//상속의 장점2: 각 과목에 대한 report()메소드를 만들지 않아도 됨.
...
class Subject{//과목
	...
	Subject(){ //점수를 지정하지 않고 과목을 생성할 경우 점수를 랜덤하게 설정
		this.score = (int)(Math.random()*101);
	}
	
	Subject(int score){//점수를 지정하여 과목을 생성할 경우 점수 필드를 해당 점수로 설정
		this.score = score;
	}
